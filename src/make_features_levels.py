...
Запуск:
    ...
    python src/make_features_levels.py --direction long --horizon 3
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--horizon",   type=int, default=3,
                   help="Баров вперёд (default: 3 мин)")
...
def main() -> None:
    ...
    print("Строим уровневые признаки...")
    ...
    out = DATA_DIR / f"dataset_levels_{args.tf}_h{args.horizon}_{args.direction}.parquet"
    df.to_parquet(out)
    ...
    print(f"~{total/days:.0f} баров/день  |  Признаков: {len(feature_cols)}")
    print(f"\nСохранено: {out}")
