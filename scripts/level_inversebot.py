"""
ИНВЕРСНЫЙ торговый бот XAUUSD на уровневых моделях.

Логика ПОЛНОСТЬЮ обратна level_bot.py:
  - Сигнал LONG  → открывает SELL
  - Сигнал SHORT → открывает BUY

Используется для проверки: если инверсный бот даёт прибыль —
оригинальный даёт убыток и наоборот.

Запуск:
    python scripts/level_inversebot.py --dry-run
    python scripts/level_inversebot.py --lot 1 --threshold 0.65
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            LOG_DIR / f"level_inversebot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
            encoding="utf-8"),
    ],
)
log = logging.getLogger("level_inversebot")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="XAUUSD Level INVERSE ML Bot")
...
def open_buy(mt5lib, symbol: str, lot: float, tp: float, sl: float,
    ...
    if dry_run:
        log.info(f"[DRY-RUN] INV-BUY {lot:.2f} лот @ {ask:.2f}  SL={sl_price}  TP={tp_price}")
        return True
    ...
    request = {
        ...
        "tp":           tp_price,
        "deviation":    10,
        "magic":        20260020,   # magic = level_inversebot BUY
        "comment":      "level_inv_buy",
    ...
    if result.retcode == mt5lib.TRADE_RETCODE_DONE:
        log.info(f"Ордер: INV-BUY {lot:.2f} @ {ask:.2f}  SL={sl_price}  TP={tp_price}  ticket={result.order}")
        return True
...
def open_sell(mt5lib, symbol: str, lot: float, tp: float, sl: float,
    ...
    if dry_run:
        log.info(f"[DRY-RUN] INV-SELL {lot:.2f} лот @ {bid:.2f}  SL={sl_price}  TP={tp_price}")
        return True
    ...
    request = {
        ...
        "tp":           tp_price,
        "deviation":    10,
        "magic":        20260021,   # magic = level_inversebot SELL
        "comment":      "level_inv_sell",
        "type_time":    mt5lib.ORDER_TIME_GTC,
        "type_filling": mt5lib.ORDER_FILLING_IOC,
    }
    result = mt5lib.order_send(request)
    if result.retcode == mt5lib.TRADE_RETCODE_DONE:
        log.info(f"Ордер: INV-SELL {lot:.2f} @ {bid:.2f}  SL={sl_price}  TP={tp_price}  ticket={result.order}")
        return True
    log.error(f"Ошибка ордера: {result.retcode}  {result.comment}")
    return False
...
def run(args: argparse.Namespace) -> None:
    ...
    log.info("РЕЖИМ: ИНВЕРСНЫЙ (LONG-сигнал → SELL, SHORT-сигнал → BUY)")
    ...
    try:
        ...
        log.warning("Модель SHORT не найдена — только INV-SELL по LONG-сигналу")

    if args.lot is not None:
        log.info(f"Порог: {args.threshold}  Лот={args.lot} (фиксированный)")
    else:
        log.info(f"Порог: {args.threshold}  Риск={args.risk}%")
    ...
    while True:
        try:
            ...
            # ИНВЕРСИЯ сигналов
            signal_inv_sell = prob_long  >= args.threshold   # LONG модель → открываем SELL
            signal_inv_buy  = prob_short >= args.threshold   # SHORT модель → открываем BUY
            ...
            signal_str = ("  >>> INV-SELL <<<" if signal_inv_sell
                          else "  >>> INV-BUY <<<" if signal_inv_buy else "")
            ...
            if has_open_position(mt5lib, args.symbol):
                if signal_inv_sell or signal_inv_buy:
